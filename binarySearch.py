class binarySearch(list):
  ...
  """Boiler plate for binary search class"""
  def __init__(self,size,step):
       self.size = size
       self.step = step

       #Generate list based on size supplied
   ...
   #This method will enable us return a list from the class instantiation.
  ...
  def search(self,number):
       ...
       #First test if number we are looking for is on first index
       #Test if it's on the last index
       ...
       #Here the number we are looking for is not on the first or last index
       #Halve our list and check if the number we are looking for is at the mid-point
       #Keep halving and check if number is equal to midpoint
